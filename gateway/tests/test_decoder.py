...
from frostlog_gateway.decoder import EverfrostDecoder, decode_state
from frostlog_gateway.protocol import ProtocolError, build_frame

DEVICE_INFO = bytes.fromhex(captured.HANDSHAKE_PLAIN)


def _state(message: captured.Message) -> dict[str, Any]:
...
def test_battery_out_of_the_cooler_has_no_serial_number() -> None:
    ...
    # Left out rather than sent as null, like every other absent field.
    assert "battery_serial_number" not in payload
...
def test_a_parameter_with_no_value_is_refused_rather_than_indexed() -> None:
    ...
    # raise IndexError, which nobody catches, and the gateway would exit on a
    # message it had not yet streamed.
...
# --- the exploratory decoder (`frostlog-gateway decode`) ------------------------------


def _message(frames: list[str], **fields: Any) -> dict[str, Any]:
    return {"model": "everfrost", "address": "AA:BB", "frames": frames, **fields}
...
def test_decode_device_info_from_the_streamed_notification() -> None:
    decoded = EverfrostDecoder().decode(_message(_frames(DEVICE_INFO)))
...
def test_decode_prefers_the_body_in_the_clear_and_reports_errors() -> None:
    decoded = EverfrostDecoder().decode(_message(_frames(b"\xff"), plain="a10105"))
    assert decoded["source"] == "plain" and decoded["params"]["a1"]["uint_le"] == 5
    assert "error" in EverfrostDecoder().decode(_message(_frames(b"\xa1\x05\xaa")))
    assert "error" in EverfrostDecoder().decode(_message(["ff09"]))  # not a frame
    assert "error" in EverfrostDecoder().decode(_message([]))


def test_fragments_are_joined_before_decoding() -> None:
    first, second = b"\x12" + DEVICE_INFO[:20], b"\x22" + DEVICE_INFO[20:]
    decoded = EverfrostDecoder().decode(_message(_frames(first, second)))
    assert decoded["params"]["a2"]["ascii"] == "ESP32"


def test_typed_readings() -> None:
    decoded = EverfrostDecoder().decode(_message([], plain="a1050500002041"))
