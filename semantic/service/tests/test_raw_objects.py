...
def test_a_chunk_is_recognised_by_its_key() -> None:
    chunk = raw_objects.parse("frostlog-raw", "v1/cooler/dt=2026-09-06/000000122880.jsonl.gz")
    assert chunk is not None
    assert chunk.stream == "cooler"
    assert chunk.table == "raw_cooler"
    assert chunk.dt == date(2026, 9, 6)
    assert chunk.offset == 122880
    assert chunk.uri == "gs://frostlog-raw/v1/cooler/dt=2026-09-06/000000122880.jsonl.gz"


def test_the_events_stream_has_its_own_table() -> None:
    chunk = raw_objects.parse("frostlog-raw", "v1/events/dt=2026-09-06/000000000000.jsonl.gz")
    assert chunk is not None
...
def test_anything_else_in_the_bucket_is_ignored(name: str) -> None:
    assert raw_objects.parse("frostlog-raw", name) is None
