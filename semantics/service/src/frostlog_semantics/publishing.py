"""Putting an event on the topic the transform announces itself on.

The topic and its messages are contracts/signals.odcs.yaml. What goes on it is
each producer's own business -- this one publishes ``semantic_updated`` -- so
this module knows only that an event can say its name and turn itself into JSON.

The contract-test job has its own copy of this (contracts/job): the two deploy
separately and the contract, not a shared library, is what binds them. The one
thing both must decide the same way is what happens when no topic is configured,
or an event would be dropped in one and published in the other.
"""
